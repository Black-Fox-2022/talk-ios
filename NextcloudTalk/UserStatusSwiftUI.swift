            ...
            VStack {
                List {
                    Section(header: Text("Online status")) {
                        Button(action: {
                            isPresentingUserStatusOptions.toggle()
                        })  {
                            HStack (spacing: 10){
                        ...
                        .sheet(isPresented: $isPresentingUserStatusOptions) {
                            UserStatusOptionsSwiftUI(changed: $changed, userStatus: $userStatus)
                        }
                    }
                    Section {
                        Button(action: {
                            isPresentingUserStatusMessageOptions.toggle()
                        })  {
                            Text(userStatus.readableUserStatusMessage() == "" ? NSLocalizedString("What is your status?", comment: "") : userStatus.readableUserStatusMessage() )
                        }
                        .foregroundColor(.primary)
                        .sheet(isPresented: $isPresentingUserStatusMessageOptions) {
                ...
                .accentColor(.clear) // For hiding the chevron closing indicatior... cant get it away elseway, without using ForEach
            }
